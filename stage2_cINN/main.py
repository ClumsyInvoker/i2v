...
def trainer(cINN, encoder, epoch, data_loader, logger, optimizer, loss_func, opt):
    ...
    for image_idx, file_dict in enumerate(data_iter):
        ...
        seq = file_dict["seq"].type(torch.FloatTensor).cuda() # bs * seq_len * 3 * h(128) * w
...
def main(opt):
    ...
    for epoch in epoch_iterator:
        ...
        flow_save_dict = aux.get_save_dict(network.flow, optimizer, scheduler, epoch)

        ## Save checkpoints
        if PFVD < best_PFVD:
            torch.save(save_dict, save_path + '/checkpoint_best_val.pth')
            torch.save(flow_save_dict, save_path + '/best_cINN.pth')
            best_PFVD = PFVD
