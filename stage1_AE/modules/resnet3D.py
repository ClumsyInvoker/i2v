...
class Encoder(nn.Module):
    ...
    def __init__(self, dic):
        # remove conv1, norm1, relu1
        # change
        #   stride_t: [1, 2, 2, 2] -> [2, 2, 2, 2]
        #   stride_s: [2, 2, 2, 2] -> [1, 1, 2, 2]
        #   result: [128*128*16 -> 4*4*1] -> [128*128*16 -> 32*32*1]
        self.inplanes = dic['inplanes'] if dic['inplanes'] is not None else 64
        ...
        # kernel_conv = dic["kernel_conv"]
        # stride_conv = dic["stride_conv"]
        # channels_conv = dic["channels_conv"]
        assert len(channels) - 1 == len(stride_t)
        assert len(channels) - 1 == len(stride_s)
        block, layers = __possible_resnets[type_]()

        self.conv1   = nn.Conv3d(3, channels[0], kernel_size=(3, 7, 7), stride=(1, 1, 1), padding=(1, 3, 3), bias=False)
        ...
        # self.conv_mu, self.conv_var = [], []
        # for i in range(len(stride_conv)):
        #    channels_in = channels_conv[i-1] if i > 0 else channels[-1]
        #    channels_out = channels_conv[i] if i < len(stride_conv) - 1 else z_dim
        #    self.conv_mu.append(nn.Conv2d(channels_in, channels_out, kernel_conv[i], stride_conv[i], 0))
        #    self.conv_var.append(nn.Conv2d(channels_in, channels_out, kernel_conv[i], stride_conv[i], 0))

        # self.conv_mu = nn.Sequential(*self.conv_mu)
        # self.conv_var = nn.Sequential(*self.conv_var)
        self.conv_mu = nn.Conv2d(channels[-1], z_dim, 3, 1, 1)
        self.conv_var = nn.Conv2d(channels[-1], z_dim, 3, 1, 1)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, stride_t=1):
        downsample = None
        if stride != 1 or stride_t != 1 or self.inplanes != planes * block.expansion:
