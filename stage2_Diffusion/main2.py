...
import stage2_Diffusion.modules.DDPM_UNet as DDPM
...
def trainer(model, encoder, generator, epoch, data_loader, logger, optimizer, loss_func, opt):
    ...
    for image_idx, file_dict in enumerate(data_iter):
        ...
        z = model(x_recon, cond, reverse=True).view(seq.size(0), -1)
        seq_gen = generator(seq[:, 0], z)
        seq_orig = seq

        loss = loss_func(x_recon, noise, seq_orig, seq_gen, logger, mode='train')
...
def validator(model, encoder, generator, epoch, data_loader, logger, loss_func, opt):
    _ = model.eval()
    logger.reset()
    data_iter = tqdm(data_loader, position=2)
    inp_string = 'Epoch {} || Loss: --- '.format(epoch)
...
def main(opt):
    ...
    denoise_model = None
    network = DDPM.GaussianDiffusion(in_channels=opt.DDPM["in_channels"],
                                     condition_dim=opt.Conditioning_Model['z_dim'],
                                     denoise_model=denoise_model,
                                     image_size=opt.DDPM['image_size'],
                                     model_channels=opt.DDPM['model_channels'],
                                     out_channels=opt.DDPM['out_channels'],
                                     num_res_blocks=opt.DDPM['num_res_blocks'],
                                     attn_resolutions=opt.DDPM['attn_resolutions'],
                                     timesteps=opt.DDPM['timesteps'],
                                     loss_type=opt.DDPM['loss_type'],
                                     betas=opt.DDPM['beta'],
    ...
    ###### Define Optimizer
    loss_func = loss.DiffusionAllLoss()
    ...
    ## Offline logging
    logging_keys = ["Loss", "L2_Loss", "Recon_Loss", 'PFVD']
    ...
    for epoch in epoch_iterator:
        ...
        trainer(network, encoder, generator, epoch, train_data_loader, loss_track_train, optimizer, loss_func, opt)

        ###### Validation #########
        epoch_iterator.set_description('Validating...')
        validator(network, encoder, generator, epoch, eval_data_loader, loss_track_test, loss_func, opt)
