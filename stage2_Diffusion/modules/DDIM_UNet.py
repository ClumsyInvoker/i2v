...
from stage2_Diffusion.modules.UNet import UNet
...
class GaussianDiffusion(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.image_size = kwargs["image_size"] # the size of z
        model_channels = kwargs["model_channels"]
        out_channels = kwargs["out_channels"]
        num_res_blocks = kwargs["num_res_blocks"]
        attn_resolutions = kwargs["attn_resolutions"]
        ...
        self.denoise_model = UNet(self.image_size, self.in_channels, model_channels, out_channels, num_res_blocks, attn_resolutions, context_dim=self.condition_dim)
    ...
    def p_mean_variance(self, x, t, clip_denoised=True, condition_tensor=None, use_broadcat=False):
        ...
        denoise_model_output = self.denoise_model(denoise_model_input, t, c)
    ...
    def p_losses(self, x_start, t, noise=None, condition_tensor=None, use_broadcat=False):
        ...
        x_recon = self.denoise_model(x_noisy, t, c)
    ...
    def forward(self, x, xc=None, reverse=False, *args, **kwargs):
        ...
        if image_size is not None:
            x = x.reshape(b, -1, image_size, image_size)
    ...
    def get_x_recon_and_noise(self, x_start, t, noise=None, condition_tensor=None, use_broadcat=False):
        noise = noise if noise is not None else torch.randn_like(x_start)
    ...
    def get_ddim_timestep_seq(self, method, timesteps, ddim_timesteps):
        if method == 'uniform':
            c = timesteps // ddim_timesteps
            ddim_timestep_seq = np.asarray(list(range(0, timesteps, c)))
        elif method == 'quad':
            ddim_timestep_seq = (
                    (np.linspace(0, np.sqrt(timesteps * .8), ddim_timesteps)) ** 2
            ).astype(int)
        else:
            raise NotImplementedError(f'There is no ddim discretization method called "{method}"')

        return ddim_timestep_seq

    def reverse(self, x_t, condition_tensor=None, use_broadcat=False):
        b = x_t.size(0)
        device = x_t.device
        ...
        ddim_timestep_seq = self.get_ddim_timestep_seq(ddim_discr_method, self.num_timesteps, ddim_timesteps)
        ...
        for i in reversed(range(0, ddim_timesteps)):
            ...
            pred_noise = self.denoise_model(denoise_model_input, t, c)

            pred_x0 = (denoise_model_input - torch.sqrt((1. - alpha_cumprod_t)) * pred_noise) / torch.sqrt(alpha_cumprod_t)
            if clip_denoised:
            ...
            x_prev = torch.sqrt(alpha_cumprod_t_prev) * pred_x0 + pred_dir_xt + sigmas_t * torch.randn_like(denoise_model_input)

            denoise_model_input = x_prev

        return denoise_model_input
